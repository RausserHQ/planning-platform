"""Execute only the root-job-bound bounded partial-replan envelope."""
...
def _trusted_envelope(event: dict[str, Any]) -> EventEnvelope:
    ...
    expected = replan_affected_subgraph_envelope(
        plan_id=envelope.subject.plan_id or "",
        base_plan_version=envelope.subject.plan_version or 0,
        affected_node_keys=tuple(payload.get("affected_node_keys", ())),
        reason=str(payload.get("reason", "")),
        delivery_id=stable_delivery,
        occurred_at=envelope.occurred_at,
    )
    if envelope != expected:
        raise RuntimeError("bounded replan envelope is not bound to this Windmill root job")
    return envelope
